...
class Logger implements LoggerInterface
{
	 ...
	 * @return boolean
	 */
	public function info($message, array $context = [], $name_file = FALSE): bool
	{
		return $this->log('info', $message, $context, $name_file);
	}
	 ...
	 * @return boolean
	 */
	public function log($level, $message, array $context = [], $name_file = FALSE): bool
	{
		if (is_numeric($level))
		{
		...
		foreach ($this->handlerConfig as $className => $config)
		{
			...
			if (! $handler->setDateFormat($this->dateFormat)->handle($level, $message, $name_file))
			{
				break;
			}
		}
